	...
	<main
		...
		<nav
			...
			<div
				...
				<div class="mt-2 grow ml-auto sm:mt-0 sm:mr-6 md:mr-0 lg:basis-auto">
					<ul
							...
							<div class="p-0 overflow-x-auto">
								<table
									...
									<thead class="align-bottom">
										<tr>
											<th
												...
												User</th>
											<th
												class="px-6 py-3 pl-2 font-bold text-left uppercase align-middle bg-transparent border-b border-gray-200 shadow-none text-size-xxs border-b-solid tracking-none whitespace-nowrap text-slate-400 opacity-70 text-center">
												Role</th>
											<th
									...
									<tbody>
										...
										PagingUserDao paging = new PagingUserDao();
										int numPage = paging.getNumPage();
										request.setAttribute("indexPage", indexPage);
										request.setAttribute("numPage", numPage);
										request.setAttribute("listUser", paging.getUserFIndex(indexPage));
										%>
											...
											<tr>
												<td
													...
													<div class="flex px-2 py-1">
														...
														<div class="flex flex-col justify-center">
															<h6 class="mb-0 leading-normal text-size-base">${item.getName()}</h6>
															<p class="mb-0 leading-tight text-size-sm text-slate-400">
																${item.getEmail()}</p>
														</div>
													</div>
												</td>
												<td
													class="p-2 align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
													<c:choose>
														<c:when test="${item.isRole()==true}">
															<p
																class="mb-0 font-semibold leading-tight text-size-base text-center">
																Admin</p>
														</c:when>
														<c:otherwise>
															<p
												...
												<td
													...
													<span
													class="font-bold leading-tight text-size-lg text-slate-400 text-blue-800">
														<c:choose>
															<c:when test="${item.getGender()==true}">
												...
												<c:choose>
													<c:when test="${item.isRole()==true}">
														<td
															class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
															<a href="editUser.jsp?"
															style="pointer-events: none; color: #eee"
															class="inline-block font-semibold text-size-sm text-slate-400 x-2 py-3 mb-0 text-center align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-size-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 text-slate-700">
																<i class="mr-2 fas fa-pencil-alt"></i> Edit
														</a> <a href="javascript:;"
															style="pointer-events: none; color: #eee"
															class="relative z-10 font-semibold inline-block px-2 py-3 mb-0 text-center text-transparent align-middle transition-all border-0 rounded-lg shadow-none cursor-pointer leading-pro text-size-xs ease-soft-in bg-150 bg-gradient-red hover:scale-102 active:opacity-85 bg-x-25 bg-clip-text">
																<i
																class="mr-2 far fa-trash-alt bg-150 bg-gradient-red bg-x-25 bg-clip-text"></i>
																Delete
														</a>
														</td>
													</c:when>
													<c:otherwise>
														<td
															class="p-2 text-center align-middle bg-transparent border-b whitespace-nowrap shadow-transparent">
															<a href="editUser.jsp?userName=${item.getNameUser()}"
															class="inline-block font-semibold text-size-sm text-slate-400 x-2 py-3 mb-0 text-center align-middle transition-all bg-transparent border-0 rounded-lg shadow-none cursor-pointer leading-pro text-size-xs ease-soft-in bg-150 hover:scale-102 active:opacity-85 bg-x-25 text-slate-700">
																<i class="mr-2 fas fa-pencil-alt text-slate-700"></i>
																Edit
														</a> <a
															href="DeleteUserServlet?userName=${item.getNameUser()}"
															class="relative z-10 font-semibold inline-block px-2 py-3 mb-0 text-center text-transparent align-middle transition-all border-0 rounded-lg shadow-none cursor-pointer leading-pro text-size-xs ease-soft-in bg-150 bg-gradient-red hover:scale-102 active:opacity-85 bg-x-25 bg-clip-text">
																<i
																class="mr-2 far fa-trash-alt bg-150 bg-gradient-red bg-x-25 bg-clip-text"></i>
																Delete
						...
						<div class="flex justify-center">
							<nav aria-label="Page navigation example">
								<ul class="flex list-style-none text-size-sm">
										...
										<c:choose>
											<c:when test="${loop.index==requestScope.indexPage}">
												<li class="page-item"><a
													...
													href="users.jsp?indexPage=${loop.index}"
											...
											<c:otherwise>
												<li class="page-item"><a
													class="page-link relative block py-1.5 px-3 border-0 bg-transparent outline-none transition-all duration-300 rounded-full text-gray-800 hover:text-gray-800 hover:bg-gray-200 focus:shadow-none"
													href="users.jsp?indexPage=${loop.index}">${loop.index}</a></li>
											</c:otherwise>
										</c:choose>
									...
									<li class="page-item"><a
										class="page-link relative block py-1.5 px-3 border-0 bg-transparent outline-none transition-all duration-300 rounded-full text-gray-800 hover:text-gray-800 hover:bg-gray-200 focus:shadow-none"
										href="#"><i class="bx bx-chevrons-right"></i></a></li>
								</ul>
							</nav>
						</div>
					</div>
